...
export default function WoWButton({
  children,
  variant = "default",
  size = "sm",
  className = "",
  ...
  const base = "relative inline-flex items-center justify-center gap-1.5 font-wow-heading tracking-wide transition-all duration-150 select-none";
  ...
  return (
    <button
      className={`${base} ${sizes[size]} ${variants[variant]} rounded-sm ${className}`}
      {...props}
    >
      {children}
    </button>
